...
| Docker Compose 本地栈 | `docker compose up --build` | 同时启动 PyStreamASR 和 MySQL，模型和日志保留在宿主机。 |
...
Compose 是额外的本地部署方式。它会启动一个 PyStreamASR 容器和一个 MySQL 8.4 容器，MySQL 数据保存在 `mysql-data` Docker volume 中，`./models` 会以只读方式挂载进应用容器，运行日志写入 `./logs`。

如果 `models/sherpa-onnx-streaming-paraformer-bilingual-zh-en/` 下还没有 Sherpa-onnx 模型，可以通过辅助容器下载：
...
现有 `.env` 仍用于非容器的本地运行。Compose 应用容器的运行环境由 `docker-compose.yml` 提供，不需要读取本地 `.env` 中的密钥。MySQL 默认只在 Compose 网络内可访问；如需检查数据库，可执行 `docker compose exec mysql mysql -upystreamasr -ppystreamasr pystreamasr`。

Windows 或 Linux 安装脚本完成后，可使用：
...
├── Dockerfile         # ASR 应用容器镜像
├── docker-compose.yml # 本地应用 + MySQL Compose 栈
